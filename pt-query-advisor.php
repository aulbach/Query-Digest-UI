<?php
	...
	$advisorRules = array(
		...
		'JOI.004' => 'Warn: Exclusion join uses wrong column in WHERE. The exclusion join (LEFT OUTER JOIN with a WHERE clause that is satisfied only if there is no row in the right-hand table) seems to use the wrong column in the WHERE clause. A query such as Ó... FROM l LEFT OUTER JOIN r ON l.l=r.r WHERE r.z IS NULLÓ probably ought to list r.r in the WHERE IS NULL clause.',
